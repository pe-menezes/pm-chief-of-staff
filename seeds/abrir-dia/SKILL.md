...
2. **Agenda**: se existir `.claude/skills/_shared/capacidades.md`, ler a linha `agenda do dia` e puxar os eventos de hoje pelo provider de lá. Sem o módulo, com essa linha vazia, ou se a busca falhar (conector expirado, máquina sem ele), perguntar em 1 linha: "como tá tua agenda hoje? (reuniões e janelas livres)". A abertura do dia não para por causa da agenda.
